...
func TestMacaroonConstrainer(t *testing.T) {
	...
	_, err := Constrain(mac, 2*24*time.Hour)
	assert.Error(t, err)

	constrainedMac, err := Constrain(mac, 2*time.Hour)
	assert.NoError(t, err)
	assert.NotEqual(t, constrainedMac, mac, "contained macaroon should be different than original")

	constrainedMac2, err := Constrain(mac, 2*time.Hour)
	assert.NoError(t, err)
...
func TestRuneConstrainer(t *testing.T) {
	...
	_, err := Constrain(rune, 2*24*time.Hour)
	assert.Error(t, err)

	constrainedRune, err := Constrain(rune, 2*time.Hour)
	assert.NoError(t, err)
